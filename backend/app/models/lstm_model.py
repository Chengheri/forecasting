...
class LSTMForecastModel(BaseForecastingModel):
    ...
    def prepare_data(self, data: Union[pd.DataFrame, pd.Series, np.ndarray]) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            # Scale features
            if self.scaler and self.config['preprocessing'].get('scale_features', True):
                X_scaled = self.scaler.fit_transform(X)
            else:
                X_scaled = X.values
            # Create sequences for LSTM
            seq_length = self.config['model'].get('sequence_length', 24)
    ...
    def fit(self, data: Union[pd.Series, pd.DataFrame, np.ndarray], 
        ...
        try:
            ...
            if optimizer_name == 'adam':
                ...
                optimizer = optim.SGD(
                    self.lstm_model.parameters(), 
                    lr=self.config['model'].get('learning_rate', 0.01),
            ...
            loss_fn_name = self.config['model'].get('loss_function', 'mse').lower()
            if loss_fn_name == 'mse':
            ...
            clip_value = self.config['model'].get('clip_gradient', 1.0)
            
            # Use early stopping if configured
            early_stopping = self.config['training'].get('early_stopping', False)
            ...
            train_ratio = 1.0 - self.config['training'].get('validation_size', 0.2)
            ...
            logger.info(f"Training LSTM model with {epochs} epochs, batch size {batch_size}")
            ...
            for epoch in range(epochs):
                ...
                for X_batch, y_batch in train_loader:
                    ...
                    loss.backward()
                    if clip_value > 0:
                        torch.nn.utils.clip_grad_norm_(self.lstm_model.parameters(), clip_value)
                    optimizer.step()
            ...
            with torch.no_grad():
                self.lstm_model.eval()
                final_train_preds = self.lstm_model(X_train).squeeze().numpy()
                final_val_preds = self.lstm_model(X_val).squeeze().numpy()
    ...
    def predict(self, data: pd.DataFrame) -> Tuple[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
        ...
        """
        if not self._model_is_fitted or self.lstm_model is None:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                for i in range(steps):
                    # Predict next step
                    output = self.lstm_model(current_input).numpy()[0, 0]
    ...
    def save(self, path: str) -> None:
        ...
        try:
            ...
            save_dict = {
                'model_state_dict': self.lstm_model.state_dict() if self.lstm_model else None,
                'config': self.config,
                'scaler': self.scaler,
    ...
    def load(self, path: str) -> None:
        ...
        try:
            ...
            # Load model weights
            if self.lstm_model and save_dict['model_state_dict']:
                self.lstm_model.load_state_dict(save_dict['model_state_dict'])
    ...
    def cross_validation(self, data: Union[pd.Series, pd.DataFrame, np.ndarray], 
        ...
        try:
            ...
            if sequence_length is None:
                sequence_length = self.config['model'].get('sequence_length', 24)
            ...
            for fold in range(n_splits):
                ...
                with torch.no_grad():
                    self.lstm_model.eval()
                    val_preds = self.lstm_model(X_val).squeeze().numpy()
